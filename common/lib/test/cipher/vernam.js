...
import vernam from '../../src/cipher/vernam';
  ...
  describe(`regression validation`, function () {
    it(`default hash algorithm`, function () {
      assert.equal(`C1n25Cu52$`,
          encode(`abcdefghij`, vernam.hash(`abcdefghij`)));
      assert.equal(`86Kl1W1dql4vN29vm056`,
          encode(`lzFT95rQGbfba5GAedoX`, vernam.hash(`lzFT95rQGbfba5GAedoX`)));
      assert.equal(`h00rzbb15Zwr42WmN92t`,
          encode(`OYb2Xlnc1FG5TgWZWPWi`, vernam.hash(`EVDM62G85RrOlJppDTpF`)));
      assert.equal(`tt@lWfjW7h662K1Wnuzn`,
          encode(`mlHINlaW5uv7xCiszlCV`, vernam.hash(`vFniW7aivKX8sydozDmB`)));
    });
